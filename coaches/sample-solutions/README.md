...
Production-quality reference implementations for all 14 hackathon challenges.
...
**Multi-file challenges** (3-01 The Relay, 3-05 Ship It) contain multiple workflow
definitions in one file, separated by dividers. Participants should copy each section
...
> **Never copy-paste for a squad.** The act of typing forces engagement with every line.
> If they type it without understanding, they will fail the next challenge anyway.
...
- **`engine: copilot` is the default** for all Track 1 and 2 challenges. Copilot is
  ...
  squads experiment with `claude` for any challenge that involves multi-step reasoning.

- **`claude` is used for Track 3 challenges** (3-04 The Overseer) where nuanced
  multi-workflow correlation reasoning benefits from Claude's capabilities.

- **`noop` is always present** in `safe-outputs` and `{{#runtime-import shared/noop-reminder.md}}`
  appears at the bottom of every workflow body. This is mandatory — see dossier pitfall #1.

- **Minimal permissions** are declared throughout. `contents: read` is the default;
  write permissions are added only where a specific safe-output requires them.

- **`checkout: false`** is used on all event-driven workflows that read only GitHub
  API data (triage, moderation, welcome). This saves 30–60 s of startup time.

---

*Generated by Vasquez (Workflow Engineer) — 2026-05-28*
